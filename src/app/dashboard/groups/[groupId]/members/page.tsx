  ...
  return (
    <div key={member.userId} className="flex items-center gap-4">
      <div className={cn(cardStyles, 'flex gap-4 hover:underline items-center p-4')}>
        ...
        {showActions && <MemberCardActions userRole={member.role} groupId={groupId} userId={member.userId} />}
      </div>
    </div>
  )
}
    ...
    <div className="space-y-8">
      ...
      {admins.length > 0 && (
        <>
          ...
          <div className="flex flex-wrap gap-4">
            {admins.map((member) => (
              <MemberCard groupId={groupId} showActions={isGroupOwner} key={member.userId} member={member} />
            ))}
          </div>
        </>
      )}
