...
export async function GroupHeader({ group }: { group: Pick<Group, 'name' | 'id' | 'isPublic' | 'bannerId'> }) {
  const user = await getCurrentUser()
            ...
            <div className="space-y-2 sm:space-y-4 text-center sm:text-left">
              ...
              <div className="flex flex-wrap justify-center sm:justify-start items-center gap-2 sm:gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs sm:text-sm font-medium">{upcomingEvents.length} upcoming events</span>
                </div>
                ...
                {!group.isPublic && (
                  <Badge
                    ...
                    variant={'outline'}
                  >
                    <LockIcon className={cn(btnIconStyles, 'h-3 w-3 sm:h-4 sm:w-4')} /> Private Group
                  </Badge>
                )}
              </div>
            </div>
