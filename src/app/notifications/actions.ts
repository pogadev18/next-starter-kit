...
export const markAllNotificationsAsReadAction = authenticatedAction.createServerAction().handler(async ({ ctx }) => {
  await markAllNotificationsAsReadUseCase(ctx.user)
  revalidatePath('/notifications')
  revalidatePath('/', 'layout')
})

export const clearReadNotificationsAction = authenticatedAction.createServerAction().handler(async ({ ctx }) => {
