...
export default async function BrowsePage({ searchParams }: { searchParams: { search?: string; page?: string } }) {
...
async function GroupsList({ search, page }: { search?: string; page: number }) {
  ...
  return (
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
        {data.map((group) => (
          <GroupCard memberCount={group.memberCount.toString()} key={group.id} group={group} buttonText="View" />
        ))}
      </div>
