...
export default async function GroupsContent({ params }: { params: { userId: string } }) {
  const { userId } = params
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
        {userGroups.map((group) => (
          <GroupCard memberCount={group.memberCount.toString()} group={group} key={group.id} buttonText="View Group" />
        ))}
      </div>
    </div>
  )
