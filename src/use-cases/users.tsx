...
import { createUser, deleteUser, getUserByEmail, updateUser, verifyPassword } from '@/data-access/users'
import { UserId, UserSession } from '@/use-cases/types'
import { createUUID } from '@/util/uuid'
import { getFileUrl, uploadFileToBucket } from '@/lib/files'
import { env } from '@/env'
import { createAccount, createAccountViaGithub, createAccountViaGoogle, updatePassword } from '@/data-access/accounts'
...
import { createPasswordResetToken, deletePasswordResetToken, getPasswordResetToken } from '@/data-access/reset-tokens'
import { ResetPasswordEmail } from '@/emails/reset-password'
import { createVerifyEmailToken, deleteVerifyEmailToken, getVerifyEmailToken } from '@/data-access/verify-email'
import { VerifyEmail } from '@/emails/verify-email'
import { getNotificationsForUser, getTop3UnreadNotificationsForUser } from '@/data-access/notifications'
...
export async function registerUserUseCase(email: string, password: string) {
  ...
  try {
    ...
    console.error('Verification email would not be sent, did you setup the resend API key?', error)
  }
...
export async function getProfileImageUrlUseCase({ userId, imageId }: { userId: UserId; imageId: string }) {
...
export async function resetPasswordUseCase(email: string) {
  ...
  await sendEmail(email, `Your password reset link for ${applicationName}`, <ResetPasswordEmail token={token} />)
}
