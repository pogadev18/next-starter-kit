...
import { assertAdminOrOwnerOfGroup, assertEventExists, assertGroupExists, assertGroupVisible } from './authorization'
import { createEvent, deleteEvent, getEvent, getEventsByGroupId, updateEvent } from '@/data-access/events'
...
export async function getEventsUseCase(authenticatedUser: UserSession | undefined, groupId: GroupId) {
  await assertGroupVisible(authenticatedUser, groupId)
  const events = await getEventsByGroupId(groupId)
  return events
}

export async function getUpcomingEventsUseCase(authenticatedUser: UserSession | undefined, groupId: GroupId) {
  await assertGroupVisible(authenticatedUser, groupId)
  const events = await getEventsByGroupId(groupId)
...
export async function deleteEventUseCase(authenticatedUser: UserSession, { eventId }: { eventId: number }) {
