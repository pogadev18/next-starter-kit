...
export async function sendInviteUseCase(
  ...
  await sendEmail(email, 'You have been invited to join a group', <InviteEmail group={group} token={invite.token} />)
}

export async function acceptInviteUseCase(authenticatedUser: UserSession, { token }: { token: string }) {
