...
export async function getReplyCountOnPost(postId: number) {
  const [{ count: total }] = await database.select({ count: count() }).from(reply).where(eq(reply.postId, postId))
  return total
}
...
export async function updateReply(replyId: number, updatedRelpy: Partial<Reply>) {
  const [updatedReply] = await database.update(reply).set(updatedRelpy).where(eq(reply.id, replyId)).returning()
