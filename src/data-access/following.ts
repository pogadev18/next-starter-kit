...
export async function createFollow(newFollow: { userId: UserId; foreignUserId: UserId }) {
  const [follow] = await database.insert(following).values(newFollow).onConflictDoNothing().returning()
  return follow
}

export async function deleteFollow(userId: UserId, foreignUserId: UserId) {
  await database.delete(following).where(and(eq(following.userId, userId), eq(following.foreignUserId, foreignUserId)))
}
