...
export async function removeMembership(userId: UserId, groupId: number) {
  await database.delete(memberships).where(and(eq(memberships.userId, userId), eq(memberships.groupId, groupId)))
}
...
export async function updateMembership(membershipId: number, updatedMembership: Partial<Membership>) {
